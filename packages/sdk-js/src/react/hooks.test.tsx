...
import { useEtherPly } from './useEtherPly';
import { EtherPlyProvider, useEtherPlyContext } from './context';
...
describe('useEtherPly', () => {
    ...
    it('initializes with IDLE status', () => {
        const { result } = renderHook(() => useEtherPly(defaultConfig));
...
describe('EtherPlyProvider', () => {
    ...
    it('provides client to children via context', async () => {
        const wrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
            return React.createElement(EtherPlyProvider, { config: defaultConfig, children });
        };

        const { result } = renderHook(() => useEtherPlyContext(), { wrapper });
    ...
    it('throws without provider', () => {
        ...
        expect(() => {
            renderHook(() => useEtherPlyContext());
        }).toThrow('useEtherPlyContext must be used within an EtherPlyProvider');
    ...
    it('auto-connects when autoConnect is true', async () => {
        const wrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
            return React.createElement(EtherPlyProvider, { config: defaultConfig, autoConnect: true, children });
        };

        renderHook(() => useEtherPlyContext(), { wrapper });
